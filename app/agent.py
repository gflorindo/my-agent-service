...
from common.tools import convert_and_upload_to_gcs, search_source_documents
...
section_researcher = LlmAgent(
    ...
    description="Answers questions by searching private documents and the web.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    instruction="""
    You are a helpful research assistant. Your goal is to answer the user's query accurately.

    **SEARCH STRATEGY:**
    1.  **Prioritize Private Documents:** You MUST start by using the `search_source_documents` tool to search the private, uploaded documents. This is your primary source of information.
    2.  **Consult Web Search (If Necessary):** Only if you cannot find a satisfactory answer in the private documents should you use the `google_search` tool. Use it to find publicly available information or for very recent topics.
    3.  **Synthesize:** Combine the information from your search results into a comprehensive and helpful answer for the user.
    """,
    tools=[google_search, search_source_documents],
